...
run_trans_enrichment <- function(
  ...
  sim_results <- bplapply(1:n_simulations, FUN = function(i) {
    ...
    random_positions <- sapply(random_chromosomes, function(chr) {
      sample(1:(chromosome_data[chromosome_data$chromosome == chr, "length"] - sequence_length + 1), 1)
    })
    ...
    sense_hits <- findOverlaps(random_gr, trans_p_gr, type = "any")
    antisense_hits <- findOverlaps(random_gr, trans_m_gr, type = "any")
    
    sense_strand_match <- strand(random_gr[queryHits(sense_hits)]) == strand(trans_p_gr[subjectHits(sense_hits)])
    antisense_strand_match <- strand(random_gr[queryHits(antisense_hits)]) == strand(trans_m_gr[subjectHits(antisense_hits)])
  ...
  plot_freq_curve_with_arrow <- function(sim_data, observed_value, title, color = "blue") {
   ...
   freq_table$count <- as.numeric(as.character(freq_table$count))
    ...
    p <- ggplot(freq_table, aes(x = count, y = freq)) +
     geom_smooth(method = "loess", se = FALSE, color = color, size = 1.2, span = 0.3) +
     ...
     theme(
       ...
       axis.title.x = element_text(size = 16, margin = margin(r = 8)),
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0OTUwNDE3NDcsMjEzNjU2ODQ3Myw4Nz
YyOTE4NzYsNjQ5NzkxNTQ5LDkzMDU3MjEwNSwtMTA0NzU4MTk3
NSwtMTk4MDczNDQ5OSwtMTQxNDM0NzAwNiwtODgxMDQwNDQxLD
E2NTQyNzU0NjldfQ==
-->