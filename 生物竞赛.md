...
##对比tsne图与umap图
#tsne图是随机分布初始化低维数据，而umap图是根据高维数据特征初始化低维数据。从而导致，tsne图注重局部结构，几乎不保留全局结构；umap图保留全局结构，弱化局部结构。UMAP更容易将相似的亚群聚集在一块，而tSNE则是注重将不同亚群区分开。
```



<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ0MzM0NjkwNSwxMDA2NzQzNjY4LC03MT
MyMzYwMDEsLTQ5NDgyNzQ1Miw2Nzk4ODYyMjIsLTE1MDI1MjQw
NzQsNTI3MTQ0ODE0LDYwNDIxNzAyOV19
-->