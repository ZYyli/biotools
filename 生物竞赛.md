...
##### 人工鉴定细胞类型建立矩阵celltype，保存在top10_rg.RData
...
##### 一些marker基因表达水平的VlnPlot图和featureplot：selectgenes_VlnPlot.png，selectgenes_FeaturePlot.png，
##
## 第一步：Cluster差异基因
...
#将人工鉴定细胞类型建立矩阵celltype，保存在top10_rg.RData
...
ggsave("selectgenes_VlnPlot.png", p1, width=12,height=20)
##VlnPlot图显示出一些marker基因在不同类型细胞中表达水平
#featureplot展示
p2 <- FeaturePlot(qcctrl,features=select_genes, reduction ="tsne",label=T,ncol=3)
ggsave("selectgenes_FeaturePlot.png", p2, width=12,height=20)
##featureplot显示出一些marker基因在30个cluster中的表达水平
#把两张图合在一起
p3=p1|p2
ggsave("selectgenes.png",p3,width=20,height=20)
```
...
##展示细胞注释结果
#按细胞类型降维画tsne图
p2 = DimPlot(qcctrl, group.by="celltype", label=T, label.size=4, reduction='tsne')
#按细胞类型降维画umap图
p3 = DimPlot(qcctrl, group.by="celltype", label=T, label.size=4, reduction='umap')
#两张图合一起。guides=“collect”删除重复的辅助线
p4 = plotc <- p1+p2+ plot_layout(guides = 'collect')
#保存三张图
ggsave("tSNE_celltype.pdf", p2, width=8,height=8)
ggsave("cell_identify/UMAP_celltype.pdf", p3, width=8,height=8)
ggsave("cell_identify/celltype.pdf", p4, width=16,height=8)
ggsave("cell_identify/celltype.png", p4, width=16,height=8)
##对比tsne图与umap图
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDU2MDY0Njg2LC0xNDkwNDEwMzIxLDEzOT
A3ODQ0ODAsLTEwNjk2MTY4Miw3MTM5NzM0ODYsODQ4MzczMjgy
LC05ODYzMzI5NDMsNDExMTkxNDExLC03NjUyMjYzOTEsMTA0OD
Q0ODU4NCwyMDQzODA4MzI1LDE2MjIzMjI2ODIsLTIwNDQ2NTMx
MzcsMTM4MjA2MjMyNCwxNjQxOTU2NTA1LDgzOTg0MDcyNCwxND
I1NDA3MDQyLC05MTg1NzM5MzksMjAxNDg1NDExOCw1Mjc1MjA0
NDBdfQ==
-->